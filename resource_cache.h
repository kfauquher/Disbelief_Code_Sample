...
struct Resource {
  ...
  Resource(std::string rid, std::vector<unsigned char> bytes)
    ...
    std::cout << "Resource " << id << " created with " << data.size()
              << " bytes\n";
  }
...
It uses weak_ptr to allow resources to be automatically removed
from the cache when they are no longer in use.
*/
class ResourceCache {
public:
  // Retrieves a resource by ID, loading it if it's not already cached or if the
  ...
  std::shared_ptr<Resource> get(const std::string &id) {
    ...
    // Simulate loading resource data (in a real engine this would read from disk
    // or another source).
  ...
  void prune() {
    for (auto it = cache_.begin(); it != cache_.end();) {
      if (it->second.expired()) {
        std::cout << "Pruning expired resource: " << it->first << '\n';
        it = cache_.erase(it);
      } else {
        ++it;
      }
